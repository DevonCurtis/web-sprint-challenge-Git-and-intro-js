...
function get20s(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    if(array[i].years.split(" - ") >= '1901' && array[i].years.split(" - ") <= '2000') {
...
function addArtist(array) {
  array.push({ 
    ...
    bio: 'Very Happy about everything'
  })
  return array;
}
...
function artistByCountry(array, nationality){
  let artist_by_country = [];
  for (let i = 0; i < array.length; i++) {
    if(array[i].nationality === nationality) {
      artist_by_country.push(array[i].name);
    }
  }
  return artist_by_country;
}
